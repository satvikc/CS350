	 ...
	 then
	    ...
	    then
		  ...
		  then
			...
			then
			   ...
			   {Interpreter '#'(['#'(ProcedureBody FEnv)] {Dictionary.entries @A} {Dictionary.entries @B})}
			   {Interpreter XS#{Dictionary.entries @A}#{Dictionary.entries @B}}
			else
	    ...
	    then
		  ...
		  local Env1 in
		     ...
		     {Interpreter '#'(C#Env1|XS {Dictionary.entries @A} {Dictionary.entries @B})}
		  end
	       else skip
	       end
	    [] V|Vs
	    then
	       if Vs==nil
	       then
		  {Interpreter '#'(V#Env|XS {Dictionary.entries @A} {Dictionary.entries @B})}
	       else
		  {Interpreter '#'(V#Env|Vs#Env|XS {Dictionary.entries @A} {Dictionary.entries @B})}
	       end
	    else skip
	    end
	 else skip
